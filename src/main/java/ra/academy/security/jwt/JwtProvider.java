...
public class JwtProvider {
    ...
    @Value("${jwt.expried.access-key}")
    private long expriedAccess;
    @Value("${jwt.expried.refresh-key}")
    private long expriedRefresh;

    public String generateAccessToken(UserDetails userDetails){
        return Jwts.builder()
                .setSubject(userDetails.getUsername())
                .setIssuedAt(new Date())
                .setExpiration(new Date(System.currentTimeMillis()+expriedAccess))
    ...
    public String generateRefreshToken(UserDetails userDetails){
        return Jwts.builder()
                .setSubject(userDetails.getUsername())
                .setIssuedAt(new Date())
                .setExpiration(new Date(System.currentTimeMillis()+expriedRefresh))
                .signWith(SignatureAlgorithm.HS512,SECRET)
                .compact();
    }
